...
def test_freeze_unfreeze():
    ...
    with pytest.raises(
        RuntimeError,
        match=re.escape("Can not set attribute a10 on frozen configuration ``ParentTestConfig``."),
    ):
        c.a10 = "1"
    c._unfreeze()
    c.a10 = "1"
    ...
    with pytest.raises(
        RuntimeError,
        match=re.escape("Can not set attribute a on a class of a frozen configuration ``Pass``."),
    ):
        c.a5.a = 0

    with pytest.raises(
        RuntimeError,
        match=re.escape("Can not set attribute a1 on frozen configuration ``SimpleConfig``."),
    ):
        c.c2.a1 = 0
    c._unfreeze()
...
def test_debug_load(tmp_path: Path, capture_logger):
    ...
    with pytest.raises(
        RuntimeError,
        match=re.escape(
            f"{ParentTestConfig4} provided args or kwargs (ParentTestConfig3(a1='10', a2=10, a8='10', a9=None,"
            " a5={'a': 10}, c2={'a1': 10}, a10='a', a6=None)) must be formatted as (args, kwargs) or (args) or"
            " (kwargs)."
        ),
    ):
